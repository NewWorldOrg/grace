...
components/
  ui/                        # shadcn/ui コンポーネント (CLI で生成、手動編集可)
  common/                    # 共通コンポーネント (DataTable, PaginatedTable 等)
...
- **Layout**: `headers()` でテーマ取得、`<html>` に `dark` クラスを SSR 時点で設定
- **ThemeProvider**: `initialMode` props で初期化、`document.documentElement` の `dark` クラスをトグル、Cookie に永続化
- **CSS カスタムプロパティ**: `globals.css` の `:root` / `.dark` セレクタで shadcn カラーパレット定義
...
- UIプリミティブは shadcn/ui (`components/ui/`) を使用
- shadcn CLI: `docker compose exec grace npx shadcn@latest add <component>`
- `components.json` で CLI のパスエイリアス設定 (`@/` ベース)
- `lib/utils.ts` に `cn()` ユーティリティ (clsx + tailwind-merge)
- アイコンは `lucide-react` を使用

#### レイアウトパターン

- ページレイアウト: `<div className="flex flex-col gap-5">`
- ヘッダー + アクション: `<div className="flex items-center justify-between">`
- セクション: shadcn `Card` + `CardHeader` + `CardContent`
- Card のネスト禁止

#### フォーム

- **`<form>` タグで囲む必須**
- shadcn `Input` + `Label` + エラー表示は `<p className="text-sm text-destructive">`
- 最初のフィールドに `autoFocus`
- バリデーション失敗時は最初のエラーフィールドにフォーカス
- 任意フィールドに `<span className="text-muted-foreground">- optional</span>`

#### ボタン

- Primary: shadcn `Button` (default variant)
...
- Tailwind の `gap-*` を使用
- セクション間: `gap-5` (20px)
- フォームフィールド間: `gap-5` (縦) / `gap-3` (横)
- ボタン間: `gap-2` (8px)

#### テーブル

- `DataTable` コンポーネント (`components/common/DataTable.tsx`)
- `columnDefinitions` API: `{ id, header, cell: (item) => ReactNode }`
- `PaginatedTable`: DataTable + ページネーション + ページサイズ選択
- 空値はハイフン (-)
- `trackBy` でアイテムID指定

#### 色・アクセシビリティ

- shadcn CSS カスタムプロパティを使用 (`text-foreground`, `bg-background` 等)
- 生の Hex 値は使用禁止（recharts のチャートカラーを除く）
- 色だけで情報を伝えない（lucide アイコン・テキスト併用）

### スタイリング

- shadcn コンポーネントのスタイルはそのまま使用
- カスタムスタイルは Tailwind ユーティリティクラスで対応
- インラインスタイルは最小限に（動的な値のみ）
...
- **TypeScript**: strict mode、`baseUrl: "."`, `paths: { "@/*": ["./*"] }`
...
PRベースの開発フローを厳守する。Claude が全工程を自律的に実行する。

### フロー

1. **開発**: 細かめのコミット
2. **PR作成**: Issue 単位
3. **セルフレビュー**: コードの品質・一貫性を確認
4. **受け入れテスト**: 以下を必ず実施
   - `task typecheck` / `task build` / `task lint` パス
   - Storybook の対象ストーリーを Playwright スクリーンショットで目視確認
   - 可能なら実アプリ (`grace.localhost`) も Playwright で確認
5. **マージ**: テスト合格後にマージ
6. **次へ**: master を pull して次のフェーズへ

### 環境ルール

- **Docker 内で実行**: `yarn`, `npx`, `build`, `lint` 等はすべて `docker compose exec grace` 経由
- **ホストマシンを汚さない**: node_modules の直接操作禁止
- **関連サービス**: Sophia (`/home/yuki/dev/src/Sophia`), Lily (`/home/yuki/dev/src/Lily`) も Docker で起動
